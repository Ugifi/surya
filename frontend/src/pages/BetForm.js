...
export default function BetForm({ game, gameType, wallet, onSubmit }) {
  ...
  return (
    <div className="bf-wrap">
      ...
      <style>{`
        .bf-wrap { background: #fff8ee; min-height: 100vh; padding-bottom: 80px; }
  ...
  box-shadow: 0 2px 12px rgba(232,160,0,0.10);
  border: 1.5px solid #ffe0a0;
}

      .bf-title {
  font-size: 16px; font-weight: 800; color: #b87200;
  margin-bottom: 10px;
  ...
  text-transform: uppercase; letter-spacing: 1px;
      ...
      .bf-desc-box {
  background: #fff8ee; border: 1px solid #f5d080;
  border-radius: 10px; padding: 9px 12px; margin-bottom: 14px;
  font-size: 13px; color: #b87200; line-height: 1.5;
}
.bf-desc-box strong { color: #8a5500; }
        ...
        .bf-label { font-size: 11px; color: #b87200; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; display: block; margin-bottom: 7px; }

        .bf-input {
  width: 100%; background: #fff8ee;
  border: 2px solid #f5d080; border-radius: 10px;
  padding: 11px 14px; color: #1a1a1a; font-size: 16px;
  outline: none; transition: border-color 0.2s, box-shadow 0.2s;
  box-sizing: border-box;
}
.bf-input:focus { border-color: #e8a000; box-shadow: 0 0 0 3px rgba(232,160,0,0.12); }
.bf-input::placeholder { color: rgba(180,120,0,0.35); }

        .bf-chips-row { display: flex; gap: 7px; flex-wrap: wrap; margin-top: 8px; }
       .bf-chip {
  background: #fff8ee; border: 1.5px solid #f5d080;
  border-radius: 8px; padding: 5px 12px;
  font-size: 13px; cursor: pointer; font-weight: 700; color: #b87200;
  transition: all 0.15s;
}
.bf-chip:hover { background: #ffe0a0; border-color: #e8a000; }
.bf-chip.active { background: linear-gradient(135deg,#e8a000,#f5a623); color: #fff; border-color: #e8a000; }
        ...
        .bf-nchip {
  background: #fff8ee; border: 2px solid #f5d080;
  border-radius: 8px; padding: 12px 6px;
  text-align: center; font-size: 16px; font-weight: 800;
  cursor: pointer; color: #b87200; transition: all 0.15s;
}
.bf-nchip:hover { border-color: #e8a000; transform: scale(1.08); }
.bf-nchip.active { background: linear-gradient(135deg,#e8a000,#f5a623); color: #fff; border-color: #e8a000; }

        .bf-jodi-scroll {
  max-height: 200px; overflow-y: auto;
  border: 2px solid #f5d080; border-radius: 10px;
  background: #fff8ee; margin-bottom: 4px;
}
.bf-jodi-scroll::-webkit-scrollbar-thumb { background: #f5a623; border-radius: 3px; }
.bf-jchip {
  padding: 10px 4px; text-align: center;
  font-size: 12px; font-weight: 700; cursor: pointer; color: #b87200;
  border-right: 1px solid #fff3d0; border-bottom: 1px solid #fff3d0;
  transition: all 0.12s;
}
.bf-jchip:hover { background: #ffe0a0; }
.bf-jchip.active { background: linear-gradient(135deg,#e8a000,#f5a623); color: #fff; }
       ...
       .bf-pana-grid::-webkit-scrollbar-thumb { background: #f5a623; border-radius: 3px; }
.bf-pchip {
  background: #fff8ee; border: 1.5px solid #f5d080;
  border-radius: 8px; padding: 8px 4px;
  text-align: center; font-size: 12px; font-weight: 700;
  cursor: pointer; color: #b87200; transition: all 0.12s;
}
.bf-pchip:hover { border-color: #e8a000; transform: scale(1.05); }
.bf-pchip.active { background: linear-gradient(135deg,#e8a000,#f5a623); color: #fff; border-color: #e8a000; }

        .bf-session-row { display: flex; gap: 10px; }
        .bf-session-btn {
  flex: 1; text-align: center; padding: 11px 0;
  font-size: 13px; font-weight: 800; cursor: pointer;
  border-radius: 10px; background: #fff8ee; color: #b87200;
  border: 2px solid #f5d080; transition: all 0.15s; letter-spacing: 1px;
}
.bf-session-btn:hover { border-color: #e8a000; }
.bf-session-btn.active { background: linear-gradient(135deg,#e8a000,#f5a623); color: #fff; border-color: #e8a000; }
        .bf-session-btn.disabled { opacity: 0.4; cursor: not-allowed; }
        .bf-session-btn.disabled:hover { border-color: #BBDEFB; }

       .bf-add-btn {
  width: 100%; background: #fff8ee; color: #b87200;
  border: 2px dashed #f5d080; border-radius: 10px;
  padding: 11px; font-weight: 700; font-size: 14px;
  cursor: pointer; margin-bottom: 14px; transition: all 0.15s;
}
.bf-add-btn:hover { background: #ffe0a0; border-color: #e8a000; }
.bf-bulk-table-wrap { overflow-x: auto; margin-bottom: 8px; border: 1.5px solid #f5d080; border-radius: 10px; }
        .bf-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.bf-table th { background: #fff8ee; color: #b87200; padding: 9px 10px; text-align: left; font-weight: 700; letter-spacing: 1px; font-size: 11px; }
.bf-table td { padding: 9px 10px; border-bottom: 1px solid #fff3d0; color: #333; }
...
.bf-total-row span   { font-size: 13px; color: #b87200; font-weight: 600; }
.bf-total-row strong { font-size: 16px; color: #8a5500; font-weight: 800; }

        .bf-place-btn {
  width: 100%;
  background: linear-gradient(135deg, #e8a000, #f5a623, #FF8C00);
...
.bf-place-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(232,160,0,0.50); }
