...
const B = {
  page: { background: '#fff8ee', minHeight: '100vh', paddingBottom: 80, color: '#222', fontFamily: '"Nunito", "Segoe UI", sans-serif' },
  header: { background: '#fff', padding: '16px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '2px solid #f5a623', position: 'sticky', top: 0, zIndex: 10, boxShadow: '0 2px 8px rgba(232,160,0,0.10)' },
  headerTxt: { fontSize: 18, fontWeight: 900, color: '#1a1a1a', letterSpacing: 1 },
  card: { background: '#fff', borderRadius: 16, border: '1.5px solid #ffe0a0', boxShadow: '0 4px 14px rgba(232,160,0,0.10)', margin: '12px', padding: '16px' },
  label: { fontSize: 11, color: '#e8a000', fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 6, display: 'block', marginLeft: 4 },
  input: { width: '100%', background: '#fff8ee', border: '2px solid #f5d080', borderRadius: 12, padding: '14px', color: '#1a1a1a', fontSize: 15, fontWeight: 600, outline: 'none', marginBottom: 16, boxSizing: 'border-box', transition: 'border 0.2s', fontFamily: 'inherit' },
  btn: { width: '100%', background: 'linear-gradient(135deg, #e8a000, #f5a623)', color: '#fff', border: 'none', borderRadius: 14, padding: 16, fontSize: 15, fontWeight: 800, cursor: 'pointer', letterSpacing: 1, textTransform: 'uppercase', boxShadow: '0 6px 20px rgba(232,160,0,0.35)' },
  badge: (color) => ({ display: 'inline-block', padding: '4px 12px', borderRadius: 20, fontSize: 10, fontWeight: 800, textTransform: 'uppercase', background: color === 'green' ? 'rgba(30,138,60,0.15)' : color === 'red' ? 'rgba(192,57,43,0.15)' : 'rgba(232,160,0,0.15)', color: color === 'green' ? '#1e8a3c' : color === 'red' ? '#c0392b' : '#e8a000' }),
};

function SubHeader({ title, onBack, rightBtn }) {
  return (
    <div style={B.header}>
      {onBack && <div onClick={onBack} style={{ fontSize: 26, cursor: 'pointer', color: '#e8a000', lineHeight: 1, fontWeight: 300 }}>‹</div>}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div onClick={onClose} style={{
          ...
          width: 32, height: 32,
          background: '#eef2f7', borderRadius: '50%',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          cursor: 'pointer', fontSize: 16, color: '#8a9bb5', fontWeight: 700,
        }}>✕</div>
        ...
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          margin: '0 auto 20px',
          boxShadow: '0 8px 24px rgba(30,138,60,0.4)',
          fontSize: 38,
        }}>✅</div>

        <div style={{ fontSize: 22, fontWeight: 900, color: '#e8a000', marginBottom: 10 }}>
        ...
        {/* Progress bar */}
        <div style={{ background: '#eef2f7', borderRadius: 20, height: 6, overflow: 'hidden', marginBottom: 10 }}>
          <div style={{
            height: '100%',
            width: `${progress}%`,
            background: 'linear-gradient(90deg,#1e8a3c,#27ae60)',
            borderRadius: 20,
            transition: 'width 0.1s linear',
          }} />
        </div>
        <div style={{ fontSize: 11, color: '#8a9bb5', marginBottom: 20, fontWeight: 600 }}>
        ...
        <button onClick={onClose} style={{
          ...
          cursor: 'pointer',
          boxShadow: '0 6px 18px rgba(30,138,60,0.35)',
        }}>
          ✕ &nbsp; Close Karo
        </button>
      </div>
      ...
      <div style={overlayStyle} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
        <div style={sheetStyle}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 22px 18px' }}>
            <div>
              <div style={{ fontSize: 20, fontWeight: 900, color: '#e8a000' }}>
          ...
          {step === 1 && (
            <div style={{ padding: '0 22px' }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12 }}>Quick Amount</div>
                ...
                {presets.map(p => (
                  <button key={p} onClick={() => setAmount(String(p))}
                    style={{
                      ...
                      background: amount === String(p) ? 'linear-gradient(135deg,#e8a000,#f5a623)' : '#fff8ee',
                      color: amount === String(p) ? '#fff' : '#e8a000',
                      border: amount === String(p) ? 'none' : '2px solid #ffe0a0',
                      boxShadow: amount === String(p) ? '0 4px 14px rgba(232,160,0,0.3)' : 'none', transition: 'all 0.2s',
                    }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 8 }}>Custom Amount</div>
              <div style={{ position: 'relative', marginBottom: 18 }}>
                <div style={{ position: 'absolute', left: 18, top: '50%', transform: 'translateY(-50%)', fontSize: 20, fontWeight: 900, color: '#e8a000' }}>₹</div>
                <input
                  style={{ width: '100%', background: '#f4f7fd', border: '2px solid #d0daea', borderRadius: 14, padding: '16px 18px 16px 40px', color: '#e8a000', fontSize: 20, fontWeight: 900, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }}
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
                {[{ l: 'Min Deposit', v: '₹100' }, { l: 'Max Deposit', v: '₹1,00,000' }].map((x, i) => (
                  <div key={i} style={{ flex: 1, background: '#eef2f7', borderRadius: 12, padding: '10px 14px', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 900, color: '#e8a000', marginTop: 2 }}>{x.v}</div>
                  </div>
                ))}
              </div>

              {msg.text && (
                <div style={{ background: '#fdecea', border: '1.5px solid #f5c6cb', borderRadius: 12, padding: '13px 16px', marginBottom: 18, color: '#c0392b', fontSize: 13, fontWeight: 700 }}>
                  {msg.text}
                </div>
              )}

              <button onClick={handleNext}
                style={{ width: '100%', background: 'linear-gradient(135deg,#e8a000,#f5a623)', color: '#fff', border: 'none', borderRadius: 16, padding: '17px', fontSize: 16, fontWeight: 900, cursor: 'pointer', letterSpacing: 1, textTransform: 'uppercase', boxShadow: '0 8px 24px rgba(232,160,0,0.35)' }}>
                PROCEED TO PAY →
              </button>
          ...
          {step === 2 && (
            <div style={{ padding: '0 22px' }}>
              <div style={{ background: 'linear-gradient(135deg,#e8a000,#e8a000)', borderRadius: 18, padding: '20px', textAlign: 'center', marginBottom: 22 }}>
              ...
              {upiId && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 10 }}>UPI ID</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#f4f7fd', border: '2px solid #d0daea', borderRadius: 16, padding: '14px 16px' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 17, fontWeight: 900, color: '#e8a000', letterSpacing: 0.3 }}>{upiId}</div>
                    </div>
                    <button onClick={copyUpi}
                      style={{ padding: '11px 18px', background: upiCopied ? 'linear-gradient(135deg,#1e8a3c,#27ae60)' : 'linear-gradient(135deg,#e8a000,#e8a000)', border: 'none', borderRadius: 12, color: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(30,79,160,0.3)', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={{ background: '#f4f7fd', borderRadius: 16, padding: '16px 18px', marginBottom: 20, border: '1.5px solid #e2e9f4' }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 14 }}>📋 Payment Steps</div>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: i < 4 ? 12 : 0, padding: s.highlight ? '10px 12px' : '0', background: s.highlight ? 'rgba(37,211,102,0.1)' : 'transparent', borderRadius: s.highlight ? 12 : 0, border: s.highlight ? '1.5px solid rgba(37,211,102,0.3)' : 'none' }}>
                    <div style={{ width: 26, height: 26, background: s.highlight ? 'linear-gradient(135deg,#25D366,#128C7E)' : 'linear-gradient(135deg,#e8a000,#e8a000)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900, fontSize: 12, flexShrink: 0 }}>{s.n}</div>
                    <div style={{ fontSize: 13, color: s.highlight ? '#128C7E' : '#e8a000', fontWeight: s.highlight ? 800 : 600, paddingTop: 3 }}>{s.t}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ fontSize: 11, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 8 }}>Transaction Number / UTR *</div>
              <input
                style={{ width: '100%', background: '#f4f7fd', border: '2px solid #d0daea', borderRadius: 14, padding: '15px 16px', color: '#e8a000', fontSize: 15, fontWeight: 700, outline: 'none', marginBottom: 16, boxSizing: 'border-box', fontFamily: 'inherit' }}
              ...
              <div style={{ background: '#eef2f7', borderRadius: 12, padding: '12px 14px', marginBottom: 18, display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ fontSize: 12, color: '#e74c3c', fontWeight: 700 }}>⏰ Approval: 0–5 hours</div>
              ...
              <button onClick={handleSubmitUTR} disabled={loading}
                style={{ width: '100%', background: loading ? '#ccc' : 'linear-gradient(135deg,#e8a000,#e8a000)', color: '#fff', border: 'none', borderRadius: 16, padding: '17px', fontSize: 16, fontWeight: 900, cursor: loading ? 'not-allowed' : 'pointer', letterSpacing: 1, textTransform: 'uppercase', boxShadow: loading ? 'none' : '0 8px 24px rgba(30,79,160,0.35)', marginBottom: 12 }}>
...
export function BidsPage({ apiCall }) {
  ...
  const statCards = [
    { icon: '🎯', val: summary.total_bids || 0, label: 'Total Bids', color: '#e8a000' },
    ...
    <div style={B.page}>
      ...
      <div style={{ padding: '14px 12px 0' }}>
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 6 }}>📅 Date Filter</div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              style={{ flex: 1, background: '#fff', border: '2px solid #d0daea', borderRadius: 10, padding: '10px 12px', color: '#e8a000', fontSize: 14, fontWeight: 600, outline: 'none', boxSizing: 'border-box' }}
            />
            {filterDate && (
          ...
          {filterDate && (
            <div style={{ fontSize: 12, color: '#e8a000', fontWeight: 700, marginTop: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 4 }}>
          {[['all', 'All'], ['pending', '⏳ Pending'], ['win', '🏆 Won'], ['loss', '💔 Lost']].map(([val, label]) => (
            <button key={val} onClick={() => setFilterStatus(val)}
              style={{ flex: 1, padding: '9px 0', borderRadius: 10, cursor: 'pointer', fontWeight: 800, fontSize: 11, background: filterStatus === val ? 'linear-gradient(135deg,#e8a000,#e8a000)' : '#fff', color: filterStatus === val ? '#fff' : '#8a9bb5', border: filterStatus === val ? 'none' : '1.5px solid #e2e9f4', transition: 'all 0.2s' }}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding: '12px 12px 8px', fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, display: 'flex', justifyContent: 'space-between' }}>
            ...
            return (
              <div key={b.id} style={{ background: '#fff', borderRadius: 14, padding: '14px', marginBottom: 10, border: '1.5px solid #e2e9f4', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 2px 8px rgba(26,58,110,0.06)', borderLeft: `4px solid ${clr}` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 800, fontSize: 14, color: '#e8a000', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{b.game_name} — {b.game_type}</div>
  ...
  return (
    <div style={B.page}>
      <SubHeader title="💳 Transactions" onBack={navigate ? () => navigate('wallet') : null}
        rightBtn={<button onClick={fetchTxns} style={{ background: '#eef2f7', border: '1px solid #d0daea', color: '#e8a000', padding: '8px 12px', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: 700 }}>🔄</button>}
      />
      ...
      <div style={{ padding: '14px 12px 0' }}>
        ...
        {filterDate && (
          <div style={{ fontSize: 12, color: '#e8a000', fontWeight: 700, marginBottom: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 10, padding: '0 12px 12px' }}>
        {[['all', 'All'], ['credit', 'Credit ⬆️'], ['debit', 'Debit ⬇️']].map(([val, label]) => (
          <button key={val} onClick={() => setFilter(val)} style={{ flex: 1, padding: '10px 0', borderRadius: 12, cursor: 'pointer', fontWeight: 800, fontSize: 12, textTransform: 'uppercase', background: filter === val ? 'linear-gradient(135deg,#e8a000,#e8a000)' : '#fff', color: filter === val ? '#fff' : '#8a9bb5', border: filter === val ? 'none' : '1.5px solid #e2e9f4', transition: 'all 0.2s', boxShadow: filter === val ? '0 4px 12px rgba(30,79,160,0.25)' : 'none' }}>{label}</button>
        ))}
      </div>
        ...
        {filtered.map((tx, i) => {
          ...
          return (
            <div key={tx.id || i} style={{ background: '#fff', borderRadius: 14, padding: '14px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 12, border: '1.5px solid #e2e9f4', borderLeft: `4px solid ${credit ? '#1e8a3c' : '#c0392b'}`, boxShadow: '0 2px 8px rgba(26,58,110,0.06)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 14, marginBottom: 3 }}>{typeLabel(tx.type)}</div>
...
export function ReferralPage({ apiCall, user, onBack }) {
  ...
  return (
    <div style={B.page}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #e8a000, #e8a000)', margin: '16px 12px', borderRadius: 20, padding: '24px 20px', textAlign: 'center', boxShadow: '0 4px 20px rgba(30,79,160,0.25)' }}>
      ...
      <div style={B.card}>
        <div style={{ fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 14 }}>🔑 Aapka Referral Code</div>
        <div style={{ background: '#eef2f7', borderRadius: 14, padding: '18px', textAlign: 'center', border: '2px dashed #e8a000', marginBottom: 14 }}>
          <div style={{ fontSize: 28, fontWeight: 900, color: '#e8a000', letterSpacing: 4 }}>
            {loading ? (
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={copyCode} style={{ flex: 1, padding: '14px', background: copied ? '#e8f5e9' : '#eef2f7', border: `1.5px solid ${copied ? '#a5d6a7' : '#d0daea'}`, borderRadius: 12, fontWeight: 800, fontSize: 14, cursor: 'pointer', color: copied ? '#1e8a3c' : '#e8a000', transition: 'all 0.2s' }}>
            {copied ? '✅ Copied!' : '📋 Copy Code'}
          </button>
          <button onClick={shareLink} style={{ flex: 1, padding: '14px', background: 'linear-gradient(135deg,#e8a000,#f5a623)', border: 'none', borderRadius: 12, fontWeight: 800, fontSize: 14, cursor: 'pointer', color: '#fff', boxShadow: '0 4px 12px rgba(232,160,0,0.3)' }}>
            🔗 Share Link
          </button>
        </div>
      </div>
      ...
      <div style={{ margin: '0 12px', background: '#fff', borderRadius: 16, border: '1.5px solid #e2e9f4', overflow: 'hidden', boxShadow: '0 4px 14px rgba(26,58,110,0.07)', marginBottom: 12 }}>
        <div style={{ padding: '14px 16px', background: '#eef2f7', borderBottom: '1px solid #e2e9f4', fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5 }}>📋 Kaise Kaam Karta Hai?</div>
        {[
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderBottom: i < 3 ? '1px solid #e2e9f4' : 'none' }}>
            <div style={{ width: 32, height: 32, background: 'linear-gradient(135deg,#e8a000,#f5a623)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900, fontSize: 14, flexShrink: 0 }}>{s.n}</div>
            <div>
              <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 14 }}>{s.t}</div>
      ...
      {!loading && referralData.referrals && referralData.referrals.length > 0 && (
        <div style={{ margin: '0 12px' }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 10 }}>👥 Mere Referrals</div>
          {referralData.referrals.map((r, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 14, padding: '14px', marginBottom: 8, border: '1.5px solid #e2e9f4', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 2px 8px rgba(26,58,110,0.06)', borderLeft: `4px solid ${r.status === 'credited' ? '#1e8a3c' : '#f0a500'}` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 14 }}>{r.joiner_name || 'User'}</div>
...
export function WalletPage({ wallet, onAdd, onWith, user, navigate, apiCall }) {
  ...
  return (
    <div style={B.page}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #e8a000, #e8a000)', padding: '28px 20px', textAlign: 'center', borderBottomLeftRadius: 24, borderBottomRightRadius: 24, boxShadow: '0 4px 20px rgba(30,79,160,0.25)', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
          <button onClick={handleAdd} style={{ flex: 1, maxWidth: 150, background: '#fff', color: '#e8a000', border: 'none', borderRadius: 30, padding: '14px 0', fontWeight: 900, fontSize: 14, cursor: 'pointer', boxShadow: '0 8px 20px rgba(0,0,0,0.2)', textTransform: 'uppercase' }}>💰 Add Money</button>
      ...
      <div style={{ background: '#fff', margin: '0 12px 16px', borderRadius: 16, border: '1.5px solid #e2e9f4', overflow: 'hidden', boxShadow: '0 4px 14px rgba(26,58,110,0.07)' }}>
        {[
          ...
          <div key={i} onClick={item.fn}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 15 }}>{item.l}</div>
              <div style={{ fontSize: 11, color: '#8a9bb5', marginTop: 3 }}>{item.sub}</div>
            </div>
            <div style={{ color: '#e8a000', fontSize: 24 }}>›</div>
          </div>
        ))}
      </div>

      <div style={{ padding: '8px 12px', fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 4 }}>📈 Your Stats</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, padding: '0 12px' }}>
        {[
          ...
          { val: String(stats.total_bids), label: 'TOTAL BIDS', color: '#e8a000' },
          { val: String(stats.games_won), label: 'GAMES WON', color: '#1e8a3c' },
          { val: '₹' + Number(stats.avg_bid).toLocaleString('en-IN'), label: 'AVG BID', color: '#e8a000' },
        ].map((s, i) => (
      ...
      <div style={{ padding: '0 12px 20px' }}>
        ...
        {[
          ...
          <div key={i} style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 10, border: '1.5px solid #e2e9f4', display: 'flex', gap: 14, boxShadow: '0 2px 8px rgba(26,58,110,0.06)' }}>
            <div style={{ width: 36, height: 36, background: 'linear-gradient(135deg,#e8a000,#f5a623)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900, fontSize: 16, flexShrink: 0, boxShadow: '0 4px 10px rgba(30,79,160,0.35)' }}>{s.n}</div>
            <div>
              <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 15, marginBottom: 4 }}>{s.t}</div>
              <div style={{ fontSize: 13, color: '#8a9bb5', lineHeight: 1.6 }}>{s.d}</div>
            </div>
          </div>
        ))}
        <div style={{ fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, margin: '20px 4px 12px' }}>🎮 Multipliers</div>
        {[
          ...
          <div key={i} style={{ background: '#fff', borderRadius: 12, padding: '14px 16px', marginBottom: 8, border: '1.5px solid #e2e9f4', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 8px rgba(26,58,110,0.06)' }}>
            <div style={{ fontWeight: 700, color: '#e8a000', fontSize: 15 }}>{g.type}</div>
  ...
  return (
    <div style={B.page}>
      ...
      <div style={{ padding: '16px 12px' }}>
        {faqs.map((f, i) => (
          <div key={i}
            style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 10, border: '1.5px solid #e2e9f4', cursor: 'pointer', boxShadow: '0 2px 8px rgba(26,58,110,0.06)', borderLeft: open === i ? '4px solid #e8a000' : '1.5px solid #e2e9f4', transition: 'all 0.2s' }}
            onClick={() => setOpen(open === i ? null : i)}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontWeight: 800, color: '#e8a000', flex: 1, paddingRight: 10, fontSize: 14, lineHeight: 1.4 }}>{f.q}</div>
              <div style={{ color: '#e8a000', fontSize: 24, fontWeight: 700, width: 24, textAlign: 'center' }}>{open === i ? '−' : '+'}</div>
            </div>
      ...
      <div style={{ padding: '16px 12px' }}>
        {items.map((s, i) => (
          <div key={i} style={{ background: '#fff', borderRadius: 14, padding: '16px', marginBottom: 10, border: '1.5px solid #e2e9f4', boxShadow: '0 2px 8px rgba(26,58,110,0.06)' }}>
            <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 14, marginBottom: 6 }}>{s.t}</div>
            <div style={{ fontSize: 13, color: '#8a9bb5', lineHeight: 1.6 }}>{s.d}</div>
          </div>
        ))}
    ...
    <div style={B.page}>
        ...
        <div style={{ margin: '0 12px 12px', background: 'linear-gradient(135deg, #fff8e1, #fff3cd)', borderRadius: 16, border: '1.5px solid #f0c040', padding: '16px', boxShadow: '0 4px 14px rgba(240,192,64,0.15)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ flex: 1, background: '#fff', borderRadius: 10, padding: '12px 14px', border: '1.5px dashed #f0c040' }}>
              <div style={{ fontSize: 22, fontWeight: 900, color: '#e8a000', letterSpacing: 3 }}>{user.referral_code}</div>
            </div>
            <button onClick={() => { navigator.clipboard.writeText(user.referral_code); }}
              style={{ padding: '12px 16px', background: '#f0c040', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer', color: '#e8a000' }}>
              📋 Copy
            </button>
          </div>
      ...
      <div style={B.card}>
        ...
        <div style={{ borderTop: '1px solid #e2e9f4', paddingTop: 20, marginTop: 8 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 16 }}>🔐 Change Password (Optional)</div>
      ...
      <div style={{ margin: '0 12px', background: '#fff', borderRadius: 16, border: '1.5px solid #e2e9f4', overflow: 'hidden', boxShadow: '0 4px 14px rgba(26,58,110,0.07)' }}>
        <div style={{ padding: '14px 16px', background: '#eef2f7', borderBottom: '1px solid #e2e9f4', fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5 }}>🎧 Help & Support</div>
        <div onClick={() => window.open(`https://wa.me/91${contacts.phone}`, '_blank')}
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 15 }}>WhatsApp Support</div>
        ...
        <div onClick={() => window.open(`https://t.me/${contacts.telegram}`, '_blank')}
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 15 }}>Telegram Support</div>
...
export function GameRatesPage({ onBack }) {
  ...
  return (
    <div style={B.page}>
      ...
      <div style={{ padding: '16px 12px' }}>
        <div style={{ background: 'linear-gradient(135deg, #e8a000, #e8a000)', borderRadius: 16, padding: '20px', textAlign: 'center', marginBottom: 20, boxShadow: '0 4px 20px rgba(30,79,160,0.25)' }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12 }}>🎯 Main Market Rates</div>
        {mainRates.map((g, i) => (
          <div key={i} style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', marginBottom: 10, border: '1.5px solid #e2e9f4', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 8px rgba(26,58,110,0.06)', borderLeft: '4px solid #f0c040' }}>
            <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 16 }}>{g.label}</div>
            <div style={{ fontWeight: 900, fontSize: 15, color: '#e8a000', background: '#eef2f7', padding: '6px 14px', borderRadius: 20 }}>{g.rate}</div>
          </div>
        ))}
        <div style={{ fontSize: 12, fontWeight: 800, color: '#e8a000', textTransform: 'uppercase', letterSpacing: 1.5, margin: '20px 0 12px' }}>⭐ Starline Game Rates</div>
        {starlineRates.map((g, i) => (
          <div key={i} style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', marginBottom: 10, border: '1.5px solid #e2e9f4', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 8px rgba(26,58,110,0.06)', borderLeft: '4px solid #e8a000' }}>
            <div style={{ fontWeight: 800, color: '#e8a000', fontSize: 16 }}>{g.label}</div>
            <div style={{ fontWeight: 900, fontSize: 15, color: '#e8a000', background: '#eef2f7', padding: '6px 14px', borderRadius: 20 }}>{g.rate}</div>
          </div>
        ))}
